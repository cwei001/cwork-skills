...
| **周期** | ⚠️ **必须确认**，通过下方「周期确认流程」自动获取 |

### 3.2 ⚠️ 周期确认流程（必须执行）

> **这是数据查询的第一步，不可跳过。** 所有 BP 数据查询都依赖 periodId，必须先确认周期。

**执行步骤**：

```
1. 调用 GET /bp/period/getAllPeriod 获取所有周期
2. 从返回结果中筛选 status = "enabled"（进行中）的周期
3. 判断进行中的周期数量：
   - 只有 1 个 → 直接使用该周期，告知用户「当前使用周期：XXX」
   - 有 2 个或更多 → 列出所有进行中的周期，请用户选择
   - 没有进行中的周期 → 列出最近的周期，请用户确认
```
...
> 🚫 **禁止行为**：不要直接问用户“你要查哪个周期？”，而是先调 API 获取周期列表，再根据结果智能处理。
...
### 示例 2：查数据（进行中周期只有 1 个）

**用户**：帮我查一下产品部的目标，Key 是 xxx

**助手**：
1. 调用 `getAllPeriod` → 发现进行中的周期只有「2026年度」
2. 自动选择「2026年度」，无需用户确认
3. 调用 `getTree` → 找到「产品部」
4. 返回产品部的目标列表
